    ...
    render(ctx) {
        ...
        for (let tx = 0; tx < OREMAP_WIDTH; tx++) {
            for (let ty = 0; ty < OREMAP_HEIGHT; ty++) {
                ...
                let tileIndex = 0;
                if (this.getFilled(tx + 0, ty + 0))
                    tileIndex += 1;
                if (this.getFilled(tx + 1, ty + 0))
                    tileIndex += 2;
                if (this.getFilled(tx + 0, ty + 1))
                    tileIndex += 4;
                if (this.getFilled(tx + 1, ty + 1))
                    tileIndex += 8;
                if (tileIndex === 15) {
