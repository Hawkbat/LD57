...
function randomOre(layer) {
    ...
    for (const oreType in oreChances) {
        cumulativeChance += oreChances[Number(oreType)] ?? 0;
    ...
    for (let layer = 0; layer < PROC_LAYERS; layer++) {
        ...
        for (let x = 0; x < LAYER_WIDTH - 1; x++) {
            for (let y = 0; y < LAYER_HEIGHT - 1; y++) {
                ...
                if (!tileMap.getFilled(x + 0, y + 0 + layer * LAYER_HEIGHT))
                    continue;
                if (!tileMap.getFilled(x + 1, y + 0 + layer * LAYER_HEIGHT))
                    continue;
                if (!tileMap.getFilled(x + 0, y + 1 + layer * LAYER_HEIGHT))
                    continue;
                if (!tileMap.getFilled(x + 1, y + 1 + layer * LAYER_HEIGHT))
                    continue;
